...
# --------------- pobieranie – BEZPIECZNE ---------------
def _download(url, dest, timeout=8):
    """Pobiera plik, zwraca True jeśli się udało."""
    try:
        subprocess.run(["wget", "--no-check-certificate", "-q", "-T", str(timeout), "-O", dest, url], check=False)
...
def get_repo_lists():
    ...
    try:
        ...
        for i in data:
            if i.get('url'):
                lst.append(("{} - {} ({})".format(i.get('name',''), i.get('author',''), i.get('version','')), "archive:{}".format(i['url'])))
    except Exception as e:
...
def get_s4a_lists():
    ...
    try:
        ...
        log("Błąd przetwarzania s4a lists: " + str(e))
...
class MyUpdaterEnhanced(Screen):
    ...
    # --------------- Listy kanałów – BEZPIECZNY WĄTEK ---------------
    def runChannelListMenu(self):
        self["info"].setText("Pobieram listy...")
        reactor.callInThread(self._bgLists)          # <-- ZAMIAST Thread(...).start()
    ...
    # --------------- Aktualizacja – BEZPIECZNY WĄTEK ---------------
    def runPluginUpdate(self):
        self["info"].setText("Sprawdzam wersję online...")
        reactor.callInThread(self._bgUpdate)         # <-- ZAMIAST Thread(...).start()
    ...
    def _bgUpdate(self):
        ...
        online = None
        try:
            if _download(ver_url, tmp_ver, timeout=5):
                with io.open(tmp_ver, 'r', encoding='utf-8') as f:
                    online = f.read().strip()
        except Exception as e:
            log("_bgUpdate exception: " + str(e))
...
def Plugins(**kwargs):
    return [PluginDescriptor(name="MyUpdater V5", description="Aktualizator list, picon i softcamów", where=PluginDescriptor.WHERE_PLUGINMENU, icon="myupdater.png", fnc=main)]
